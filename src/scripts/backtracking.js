...
function backtrackingQueens(size) {
  ...
  if (solution) { enqueue(ActionCreator.finish); render(board); }
}
...
function dfs(board, currentQueen, size) {
  ...
  for (var row = 0; row < size; row++) {
    ...
    enqueue(ActionCreator.moveQueen.bind(null, currentQueen, row));
  ...
  enqueue(ActionCreator.moveQueen.bind(null, currentQueen, null));
}
